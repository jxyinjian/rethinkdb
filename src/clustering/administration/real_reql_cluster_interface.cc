...
bool real_reql_cluster_interface_t::db_drop(
        boost::optional<auth::username_t> const &username,
        const name_string_t &database_name,
        signal_t *interruptor_on_caller,
        ql::datum_t *result_out,
        admin_err_t *error_out) {
    guarantee(database_name != name_string_t::guarantee_valid("rethinkdb"),
        "real_reql_cluster_interface_t should never get queries for system tables");
    ...
    size_t tables_dropped = 0;
    {
        ...
        if (!search_db_metadata_by_name(
                metadata.databases, database_name, &database_id, error_out)) {
            return false;
        }
        ...
        for (const auto &table_id : table_ids) {
            try {
                ...
                OK. */
            } CATCH_OP_ERRORS(database_name, table_names.at(table_id).name, error_out,
        ...
        old_config = convert_db_config_and_name_to_datum(database_name, database_id);

        metadata.databases.databases.at(database_id).mark_deleted();
